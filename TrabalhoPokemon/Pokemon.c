...
int tipos[2][3] = {{1,2,3}, {1,2,3}}; // 1=fogo, 2=água, 3=planta
...
// ======= FÓRMULA DE DANO GEN 1 ========
int calcular_dano(int atacante, int defensor, int ataque_escolhido) {
    double nivel = 50.0;

    double poder_ataque = poder[ativo[atacante]][ataque_escolhido];
    double atk = (double) ataque[atacante][ativo[atacante]];
    double def = (double) defesa[defensor][ativo[defensor]];

    // Fórmula Gen 1:
    double dano = (((((2 * nivel) / 5) + 2) * poder_ataque * (atk / def)) / 50) + 2;

    // STAB (Same Type Attack Bonus)
    if (tipo_ataque[ativo[atacante]][ataque_escolhido] == tipos[atacante][ativo[atacante]]) {
        dano *= 1.5;
    }
